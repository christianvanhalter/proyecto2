...
<?php
//PREGUNTAMOS NUESTRO NIVEL DE USUARIO: (para saber el nivel de usuario de la sesión actual)
...
{
  //si no somos administrador, entonces ejecutará una consulta donde seamos 'asesor'
  $sel = $con->prepare("SELECT * FROM clientes WHERE asesor = ? ");
  $sel->bind_param('s', $_SESSION['nombre']);
}
$sel->execute();
$res = $sel->get_result();
$row = mysqli_num_rows($res); //aquí nos dará el número de resultados que llego de la BD
?>
  ...
  <div class="col s12">
    <div class="card">
        <div class="card-content">
          ...
          <!--TABLA COMÚN PARA MOSTRAR LOS RESULTADOS DE LA SETENCIA SQL-->
          <table>
            <thead>
              <tr class="cabecera"></tr>
              <th>Nombre</th>
              <th>Dirección</th>
              <th>Teléfono</th>
              <th>Correo</th>
            ...
            <?php while($f = $res->fetch_assoc()) { ?>
              <tr>
                ...
                <td>
                  <!--Botón eliminar-->
                  <a href="#" class="btn-floating red" onclick="swal({ title: 'Esta seguro que desea eliminar al cliente?', text: 'Al eliminarlo no podrá recuperarlo!', type: 'warning', showCancelButton: true,confirmButtonColor: '#3085d6', cancelButtonColor: '#d33', confirmButtonText: 'Si, eliminarlo' }).then(function() {
